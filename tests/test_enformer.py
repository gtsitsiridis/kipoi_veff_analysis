...
def test_enformer(chr22_example_files, random_model, output_dir: Path, size, batch_size, allele_type: str):
    args = {
    ...
    run_enformer(dl, random_model, enformer_filepath, size, batch_size=batch_size)


@pytest.mark.parametrize("allele_type", [
    'REF', 'ALT'
])
def test_enformer_tissue_mapper(allele_type: str, chr22_example_files, random_model, output_dir: Path,
    ...
    if not enformer_filepath.exists():
        ...
        test_enformer(chr22_example_files, random_model, output_dir, size, batch_size, allele_type)
    else:
    ...
    tissue_mapper.predict(enformer_filepath, output_path=enformer_tissue_filepath, batch_size=batch_size)
...
def test_calculate_veff(chr22_example_files, random_model, output_dir: Path,
    ...
    if ref_filepath.exists():
        ...
        test_enformer_tissue_mapper('REF', chr22_example_files, random_model, output_dir,
    ...
    if alt_filepath.exists():
        ...
        test_enformer_tissue_mapper('ALT', chr22_example_files, random_model, output_dir,
                                    enformer_tracks_path, gtex_tissue_matcher_path, size=size)

    calculate_veff(ref_filepath, alt_filepath, output_dir / f'enformer_{size}_veff.parquet')
